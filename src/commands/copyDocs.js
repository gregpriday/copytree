...
async function copyDocsCommand(options) {
  try {
    ...
    if (options.output) {
      ...
      console.log(chalk.green(`✓ Documentation for "${topic}" copied to clipboard`));
      console.log(chalk.gray(`${docContent.split('\n').length} lines, ${docContent.length} characters`));
    } else {
      // Display to console
...
async function listAvailableTopics(docsDir) {
  ...
  if (await fs.pathExists(frameworksDir)) {
    ...
    for (const file of frameworks) {
      if (file.endsWith('.md')) {
        ...
        console.log(`  ${chalk.bold(name.padEnd(20))} - Framework-specific documentation`);
      }
    }
    console.log();
  }
  ...
  if (await fs.pathExists(topicsDir)) {
    console.log(chalk.green('Topics:'));
  ...
  // Check for built-in docs
  console.log(chalk.magenta('Built-in:'));
  console.log(`  ${chalk.bold('profiles'.padEnd(20))} - Profile system documentation`);
  console.log(`  ${chalk.bold('transformers'.padEnd(20))} - File transformer documentation`);
  console.log(`  ${chalk.bold('pipeline'.padEnd(20))} - Pipeline architecture documentation`);
  console.log(`  ${chalk.bold('configuration'.padEnd(20))} - Configuration guide`);
  console.log();
  
  console.log(chalk.gray('Usage: copytree copy:docs --topic <name>'));
}
