...
class SortFilesStage extends Stage {
  ...
  validate(input) {
    ...
    if (!validSortOptions.includes(this.sortBy)) {
      throw new Error(`Invalid sortBy option: ${this.sortBy}. Must be one of: ${validSortOptions.join(', ')}`);
    }
    
    const validOrderOptions = ['asc', 'desc'];
    if (!validOrderOptions.includes(this.order)) {
      throw new Error(`Invalid order option: ${this.order}. Must be one of: ${validOrderOptions.join(', ')}`);
    }
