...
import { PERFORMANCE_BUDGETS, getProjectSizeCategory } from '../../utils/performanceBudgets.js';
...
const StatRow = ({ label, value, color = 'white', budgetInfo }) => {
  return React.createElement(
    ...
    React.createElement(
      ...
      budgetInfo && React.createElement(Text, { color: 'gray', fontSize: 10 }, budgetInfo),
    ),
  );
};
  ...
  return React.createElement(
    ...
    stats.totalSize !== undefined &&
      React.createElement(StatRow, {
        ...
        budgetInfo: showPerformanceBudgets
          ? `budget: ${formatBytes(PERFORMANCE_BUDGETS.projectSize.max)}`
          : undefined,
      }),
    ...
    duration &&
      React.createElement(StatRow, {
        ...
        budgetInfo: showPerformanceBudgets
          ? `budget: ${formatDuration(timeBudget)} (${sizeCategory})`
          : undefined,
      }),
      ...
      React.createElement(
        ...
        ...Object.entries(stats.perStageTimings).map(([stageName, timing]) => {
          ...
          return React.createElement(StatRow, {
            ...
            budgetInfo:
              showPerformanceBudgets && timing > PERFORMANCE_BUDGETS.stageTime.warning
                ? timing > PERFORMANCE_BUDGETS.stageTime.critical
                  ? 'CRITICAL'
                  : 'SLOW'
                : undefined,
          });
