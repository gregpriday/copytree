...
const PerformanceBudget = ({ stats, duration, fileCount }) => {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(Text, { color: 'gray' }, `(${summary.sizeCategory} project)`),
    ),
    ...
    summary.warnings.length > 0 &&
      React.createElement(
        Box,
        { flexDirection: 'column', marginTop: 1 },
        React.createElement(Text, { color: 'yellow', bold: true }, '⚠ Performance Warnings:'),
        ...summary.warnings.map((warning, i) =>
          React.createElement(Text, { key: i, color: 'yellow', marginLeft: 2 }, `• ${warning}`),
        ),
      ),
