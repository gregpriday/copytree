...
jest.mock('../src/services/AIService.js', () => {
  ...
  const MockAIService = jest.fn().mockImplementation((options = {}) => ({
    config: {
      get: jest.fn().mockImplementation((key, defaultValue) => {
        // Mock some common config values
        const mockConfig = {
          'ai.gemini.model': 'gemini-1.5-flash',
          'ai.cache.enabled': true,
          'ai.cache.ttl': 86400,
          'ai.retry.maxAttempts': 3,
          'ai.retry.initialDelay': 1000,
          'ai.retry.maxDelay': 10000,
  ...
  MockAIService.forTask = (task, options = {}) => {
    // Only throw for the specific 'unknown-task' to match the original behavior
    if (task === 'unknown-task') {
      throw new Error(`Unknown AI task: ${task}`);
    }
    // For all other tasks, return a new instance (assuming they might be valid)
  ...
  MockAIService.summarizeText = async () => 'Mocked text summary';
  MockAIService.summarizeUnitTests = async () => 'Mocked unit test summary';
  MockAIService.describeSVG = async () => 'Mocked SVG description';
  MockAIService.isAvailable = () => true;
  MockAIService.getProvider = () => 'mock-provider';
  MockAIService.getModel = () => 'mock-model';

  // Mock singleton instance
  let singletonInstance = null;
  const getAI = () => {
    if (!singletonInstance) {
      singletonInstance = new MockAIService();
    }
    return singletonInstance;
  };
  ...
  return {
    ...
    summarizeFile: MockAIService.summarizeFile,
    describeImage: MockAIService.describeImage,
    summarizeText: MockAIService.summarizeText,
    summarizeUnitTests: MockAIService.summarizeUnitTests,
