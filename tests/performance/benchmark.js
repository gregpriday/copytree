...
class CopyTreeBenchmark {
  ...
  async createTestProject(options = {}) {
    ...
    console.log(
      `  Files: ${fileCount}, Depth: ${directoryDepth}, Avg Size: ${averageFileSize} bytes`,
    );
  ...
  async runBenchmark(name, testFn) {
    ...
    console.log(
      `  Memory: RSS ${this.formatBytes(memoryDelta.rss)}, Heap ${this.formatBytes(memoryDelta.heapUsed)}`,
    );
    if (error) {
  ...
  generateReport() {
    ...
    if (successfulTests > 0) {
      ...
      const totalMemoryUsed = successfulResults.reduce(
        (sum, r) => sum + Math.max(0, r.memory.heapUsed),
        0,
      );
    ...
    if (smallProjectResult) {
      console.log(
        `Small project (10 files): ${smallProjectResult.duration}ms ${smallProjectResult.duration < 1000 ? '✓' : '⚠'}`,
      );
    }
    if (mediumProjectResult) {
      console.log(
        `Medium project (100 files): ${mediumProjectResult.duration}ms ${mediumProjectResult.duration < 5000 ? '✓' : '⚠'}`,
      );
    }
    if (largeProjectResult) {
      console.log(
        `Large project (500+ files): ${largeProjectResult.duration}ms ${largeProjectResult.duration < 30000 ? '✓' : '⚠'}`,
      );
    }
    ...
    if (seqTransform && par5Transform) {
      const speedup5 = (
        ((seqTransform.duration - par5Transform.duration) / seqTransform.duration) *
        100
      ).toFixed(1);
    ...
    if (seqTransform && par10Transform) {
      const speedup10 = (
        ((seqTransform.duration - par10Transform.duration) / seqTransform.duration) *
        100
      ).toFixed(1);
    ...
    if (cache1 && cache2) {
      const cacheSpeedup = (((cache1.duration - cache2.duration) / cache1.duration) * 100).toFixed(
        1,
      );
    ...
    if (standardDiscovery && optimizedDiscovery) {
      const discoverySpeedup = (
        ((standardDiscovery.duration - optimizedDiscovery.duration) / standardDiscovery.duration) *
        100
      ).toFixed(1);
    ...
    if (bufferedOutput && streamingOutput) {
      const memoryReduction = (
        ((bufferedOutput.memory.heapUsed - streamingOutput.memory.heapUsed) /
          bufferedOutput.memory.heapUsed) *
        100
      ).toFixed(1);
