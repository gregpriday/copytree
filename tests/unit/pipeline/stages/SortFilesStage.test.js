...
describe('SortFilesStage', () => {
  ...
  describe('process', () => {
    ...
    it('should maintain file objects integrity', async () => {
      ...
      const input = {
        files: [
          { relativePath: 'test.js', content: 'test content', custom: 'prop' }
        ]
      };
  ...
  describe('validate', () => {
    ...
    it('should throw on invalid sortBy option', () => {
      ...
      expect(() => stage.validate({ files: [] }))
        .toThrow('Invalid sortBy option: invalid');
    });

    it('should throw on invalid order option', () => {
      stage = new SortFilesStage('path', 'invalid');
      expect(() => stage.validate({ files: [] }))
        .toThrow('Invalid order option: invalid');
    });
  });
